<template>
  <div class="example">
    ...
    <div class="logger">
      <template v-for="(log,index) in relayerLog" v-bind:key="'logline'+index">
        <div>{{log}}</div>
      </template>
    </div>
    <h3>Relayer Logger output above.<br/>Please open dev-tools/console log for additional details.</h3>
    <div v-if="step==1">
      <h4>Enter details for chain A</h4>
      <input v-model="config.chainA.endpoint" placeholder="RPC Endpoint"/><br/>
      <input v-model="config.chainA.addrPrefix" placeholder="Address Prefix"/><br/>
      <input v-model="config.chainA.gasPrice" placeholder="Gas Price (e.g. 0.001uatom)"/><br/>
      <textarea v-model="config.chainA.mnemonic" placeholder="Mnemonic"/><br/>
      <button v-on:click="next">Next</button>
    </div>
    <div v-if="step==2">
      <h4>Enter details for chain B</h4>
      <input v-model="config.chainB.endpoint" placeholder="RPC Endpoint"/><br/>
      <input v-model="config.chainB.addrPrefix" placeholder="Address Prefix"/><br/>
      <input v-model="config.chainB.gasPrice" placeholder="Gas Price (e.g. 0.001uatom)"/><br/>
...
export default {
  ...
  methods: {
    ...
    async relayerLoop(options={ poll: 5000, maxAgeDest: 86400, maxAgeSrc: 86400 }) {
      while (this.running) {
        try {
					this.nextRelay = await this.link.checkAndRelayPacketsAndAcks(this.nextRelay, 2, 6)
    ...
    async setupRelayer() {
      ...
      this.link = await Link.createWithNewConnections(this.clientA, this.clientB,this.logger());
